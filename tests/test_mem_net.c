...
/* perfect-memory policy: the mirror carries the pairs (v0 design), so
 * scripted players play near-optimally - short, deterministic games. */
static int perfect_pick(const mem_game *g, int slot) {
    int n = g->count;
    if (slot == 0) {
        for (int p = 0; p < n / 2; p++) {
            int a = -1, b = -1;
    ...
    /* second card: complete the pair if the mate is down */
    if (g->first >= 0) {
        int fp = g->card[g->first].pair;
        for (int i = 0; i < n; i++)
            if (i != g->first && !g->card[i].matched
                && g->card[i].state == MEM_CARD_DOWN && g->card[i].pair == fp)
                return i;
    }
    for (int i = 0; i < n; i++)
        if (!g->card[i].matched && g->card[i].state == MEM_CARD_DOWN)
...
static int play_until_win(cli *A, cli *B, game_log *lg, int timeout_ms) {
    ...
    while (now_ms() < deadline) {
        ...
        if (dt > 0.05f)
            dt = 0.05f;
        last = tnow;
        mem_step(&A->c.g, dt);   /* mirrors animate locally; decisions
        ...
        for (int k = 0; k < 2; k++) {
            ...
            if ((g->phase == MEM_PHASE_PICK1 || g->phase == MEM_PHASE_PICK2)
                ...
                int i = perfect_pick(g, g->phase == MEM_PHASE_PICK1 ? 0 : 1);
                if (i >= 0
...
int main(int argc, char **argv) {
    ...
    game_log g1;
    int rc = play_until_win(&A, &B, &g1, 45000);
    ...
    while (!(A.c.g.phase == MEM_PHASE_PICK1 && A.c.g.turn == B.c.you
        ...
        if ((B.c.g.phase == MEM_PHASE_PICK1 || B.c.g.phase == MEM_PHASE_PICK2)
            ...
            int i = perfect_pick(&B.c.g, B.c.g.phase == MEM_PHASE_PICK1 ? 0 : 1);
            if (i >= 0
        ...
        if ((A.c.g.phase == MEM_PHASE_PICK1 || A.c.g.phase == MEM_PHASE_PICK2)
            ...
            int i = perfect_pick(&A.c.g, A.c.g.phase == MEM_PHASE_PICK1 ? 0 : 1);
            if (i >= 0
    ...
    int stolen = perfect_pick(&A.c.g, 0);
    ...
    game_log g2;
    rc = play_until_win(&A, &B2, &g2, 45000);
