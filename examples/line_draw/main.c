...
static bool init_gl(void) {
    ...
    g_window = SDL_CreateWindow("A Mongoose Engine - Line Draw", g_win_w, g_win_h, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
    ...
    glBufferData_(GL_ARRAY_BUFFER, 1024 * 1024, NULL, GL_DYNAMIC_DRAW);
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv) {
    ...
    SDL_SetAppMetadata("A Mongoose Engine - Line Draw", "0.1", "com.example.ame.line_draw");
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    float mvp[16];
    // Pixel-perfect camera: top-left origin, integer-snap camera at (0,0), zoom 1
