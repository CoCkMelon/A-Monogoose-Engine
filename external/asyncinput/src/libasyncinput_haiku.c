// Haiku backend: POSIX-style device readers for keyboard and mouse.
#include "asyncinput.h"

#if defined(__HAIKU__) || defined(ASYNCINPUT_FORCE_HAIKU)
	...
	for (;;) {
		...
		for (ssize_t i = 0; i < r; i++) {
			...
			if (have >= 3) {
				...
				long long ts = now_ns();
				struct ni_event ev = {0}; ev.device_id = -21; ev.timestamp_ns = ts;
				/* buttons */
...
ni_init(int flags)
{
	...
	const char *kbd = getenv("ASYNCINPUT_HAIKU_KBD_DEVICE");
	const char *kbd2 = getenv("ASYNCINPUT_KBD_DEVICE");
	const char *mse = getenv("ASYNCINPUT_HAIKU_MOUSE_DEVICE");
	const char *mse2 = getenv("ASYNCINPUT_MOUSE_DEVICE");
	if (!kbd) kbd = kbd2;
	if (!mse) mse = mse2;

	/* Keyboard open attempts */
	if (kbd) g.kb_fd = try_open(kbd);
	if (g.kb_fd < 0) g.kb_fd = try_open("/dev/input/keyboard/0");
	if (g.kb_fd < 0) g.kb_fd = try_open("/dev/input/keyboard");
	if (g.kb_fd < 0) g.kb_fd = try_open("/dev/console");

	/* Mouse open attempts */
	if (mse) g.ms_fd = try_open(mse);
	if (g.ms_fd < 0) g.ms_fd = try_open("/dev/input/mouse/0");
	if (g.ms_fd < 0) g.ms_fd = try_open("/dev/input/mouse");
	if (g.ms_fd < 0) g.ms_fd = try_open("/dev/mouse");
	...
	if (g.ms_fd >= 0 && g.mice_enabled) {
		if (pthread_create(&g.ms_thread, NULL, ms_worker, NULL) != 0) {
			close(g.ms_fd); g.ms_fd = -1; /* optional */
		}
	}
	g.initialized = 1;
	return 0;
...
#else
/* Non-Haiku build: provide an empty TU so the file compiles if accidentally selected. */
typedef int make_cfile_nonempty;
#endif
