...
#if defined(ASYNCINPUT_STUB_SDL2) || defined(ASYNCINPUT_STUB_SDL)

/* SDL2 is required for this backend; include from either SDL2/ or flat include path. */
...
#    include <SDL.h>
#  else
#    error "SDL2 headers not found for stub backend"
#  endif
#else
#  include <SDL2/SDL.h>
#endif
...
{
    /* SDL2 provides milliseconds; convert to nanoseconds. Prefer 64-bit if available. */
#if SDL_VERSION_ATLEAST(2,0,18)
    return (long long)(SDL_GetTicks64()) * 1000000LL;
#else
    return (long long)(SDL_GetTicks()) * 1000000LL;
#endif
}
...
static int sdl_worker(void *data)
{
    ...
    SDL_Window *win = SDL_CreateWindow("asyncinput", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1, 1, SDL_WINDOW_HIDDEN);
    if (!win) return -1;

    while (SDL_AtomicGet(&g.stop) == 0) {
        SDL_Event e;
        while (SDL_PollEvent(&e)) {
            ...
            switch (e.type) {
                ...
                case SDL_MOUSEBUTTONUP: {
                    int down = (e.type == SDL_MOUSEBUTTONDOWN) ? 1 : 0;
                ...
                case SDL_KEYDOWN:
                case SDL_KEYUP:
                    ev.type = NI_EV_KEY; ev.value = (e.type == SDL_KEYDOWN) ? 1 : 0; ev.code = (int)e.key.keysym.scancode; if (g.cb) g.cb(&ev, g.cb_user); else queue_push(&ev);
                    break;
                default: break;
            }
        }
