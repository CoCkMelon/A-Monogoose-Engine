...
AmeWorldTransform2D ameComputeWorldTransform(ecs_world_t* world, ecs_entity_t e) {
    ...
    // Collect chain from root -> ... -> e
    ecs_entity_t chain[kMaxHierarchyDepth];
    int count = 0;
    ecs_entity_t cur = e;
    while (cur && count < kMaxHierarchyDepth) {
        chain[count++] = cur;
    ...
    // Walk from parent-most to the requested entity
    for (int i = count - 1; i >= 0; --i) {
        ecs_entity_t ent = chain[i];
        const AmeTransform2D* tr = (const AmeTransform2D*)ecs_get_id(world, ent, g_comp.transform);
        const unitylike::Scale2D* sc = (const unitylike::Scale2D*)ecs_get_id(world, ent, g_comp.scale2d);
