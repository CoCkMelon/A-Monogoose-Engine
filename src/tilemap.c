...
void ame_tilemap_renderer_init(void) {
    ...
    const char* tile_fs_src =
        ...
        "#define AME_COORDS_GLSL 1\n"
        "int ame_flip_y_index_top_to_bottom_gl(int y_top, int h) { return (h - 1) - y_top; }\n"
        "ivec2 ame_world_px_to_tile_index_bottom_left(vec2 world_px, ivec2 tile_size) { return ivec2(floor(world_px / vec2(tile_size))); }\n"
        "ivec2 ame_world_px_to_in_tile_px(vec2 world_px, ivec2 tile_size) { vec2 tile_frac = fract(world_px / vec2(tile_size)); return ivec2(tile_frac * vec2(tile_size)); }\n"
        "out vec4 frag;\n"
        "void main(){\n"
        ...
        "    ivec2 tcoord = ame_world_px_to_tile_index_bottom_left(world_px, tile_size);\n"
        "    if (any(lessThan(tcoord, ivec2(0))) || any(greaterThanEqual(tcoord, u_map_size))) continue;\n"
        "    ivec2 in_tile_px = ame_world_px_to_in_tile_px(world_px, tile_size);\n"
